 ...
 * <p>
 * Allow long press and drag for highlighting (Taylor Johnson)
 *
 * @author Philipp Jahoda
 */
public class RNBarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<?
    ...
    /**
     * extra state for ChartTouchListener
     */
    private static final int LONG_PRESS_DRAG = 7;

    /**
     * Can user long press and drag a highlight
     */
    private boolean mLongPressAndDragHighlightEnabled = false;

    /**
     * flag to keep track of gesture starting/ending
     * <p>
     * Helpful for determining if onLongPress fires after ACTION_UP
     */
    private boolean mGestureActive = true;
     ...
     */
    public RNBarLineChartTouchListener(BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends
     ...
     * @param dragTriggerDistance              the minimum movement distance that will be interpreted as a "drag" gesture in dp (3dp equals
     *                                         to about 9 pixels on a 5.5" FHD screen)
     * @param longPressAndDragHighlightEnabled Whether longpress and drag to move highlight is enabled
     */
    public RNBarLineChartTouchListener(BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends
            IBarLineScatterCandleBubbleDataSet<? extends Entry>>> chart, Matrix touchMatrix, float dragTriggerDistance, boolean longPressAndDragHighlightEnabled) {
        super(chart);
        this.mMatrix = touchMatrix;
        ...
        this.mLongPressAndDragHighlightEnabled = longPressAndDragHighlightEnabled;
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                if (mTouchMode == LONG_PRESS_DRAG) {
                    // Must remove highlights after another pointer is used
                    cleanUpHighlights();
                }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                } else if (mTouchMode == LONG_PRESS_DRAG) {
                    mChart.disableScroll();

                    // perform highlighting by drag
                    if (mLongPressAndDragHighlightEnabled) {
                        performHighlightDragMovableY(event);
                    }
    ...
    /**
     * Highlights when dragging, but uses the YAxis value relative to touch point not a data entry.
     */
    private void performHighlightDragMovableY(MotionEvent e) {
        Highlight h = mChart.getHighlightByTouchPoint(e.getX(), e.getY());
        Transformer transformer = mChart.getTransformer(h.getAxis());

        MPPointF clampedTouchPoint = new MPPointF(e.getX(), Math.max(Math.min(e.getY(), mChart.getViewPortHandler().contentBottom()), mChart.getViewPortHandler().contentTop()));
        MPPointD value = transformer.getValuesByTouchPoint(clampedTouchPoint.getX(), clampedTouchPoint.getY());

        Highlight lastHighlight = mLastHighlighted;

        Highlight highlight = new Highlight(h.getX(), (float) value.y, h.getXPx(), e.getY(), h.getDataSetIndex(), h.getAxis());
    ...
    private boolean shouldHighlightChange(Highlight highlight, Highlight lastHighlight) {
        return lastHighlight == null ||
                highlight.getX() != lastHighlight.getX() ||
                highlight.getY() != lastHighlight.getY() ||
                highlight.getDataSetIndex() != lastHighlight.getDataSetIndex() ||
                highlight.getStackIndex() != lastHighlight.getStackIndex() ||
                highlight.getDataIndex() != lastHighlight.getDataIndex();
    }

    /**
     * Removes Highlight when touch mode is LONG_PRESS_DRAG
     */
    private void cleanUpHighlights() {
        performHighlight(null, null);
    }
    ...
    public void onLongPress(MotionEvent e) {
        // if no touch mode is set, we haven't moved outside the drag start distance and can
        // start handling the drag events after the long press
        if (mLongPressAndDragHighlightEnabled && mGestureActive && mTouchMode == NONE) {
            mTouchMode = LONG_PRESS_DRAG;
            performHighlightDragMovableY(e);
        }
